...
class HZListenViewController: UIViewController {
    // 头部 - headerView
    private lazy var headerView:HZListenHeaderView = {
        let view = HZListenHeaderView.init(frame: CGRect(x:0, y:0, width:LBFMScreenWidth, height:120))
        return view
    }()
    // 添加控制器
    private lazy var viewControllers: [UIViewController] = {
        let listenSubscibeVC = HZListenSubscibeController()
        let listenChannelVC = HZListenChannelController()
        let listenRecommendVC = HZListenRecommendController()
...
extension HZListenViewController : LTAdvancedScrollViewDelegate {
    // 具体使用请参考以下
